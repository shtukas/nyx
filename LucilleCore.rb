...
class LucilleCore
    ...
    def self.selectEntityFromListOfEntitiesOrNull(type, elements, toStringLambda = lambda{ |item| item })
        puts "Select #{type}"
        indexDisplayMaxSize = elements.size.to_s.size # This allows adjustement of the index cube.
    ...
    def self.menuItemsWithLambdas(items)
        puts "->"
        indexDisplayMaxSize = items.size.to_s.size # This allows adjustement of the index cube.
