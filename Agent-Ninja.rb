...
class Ninja
    ...
    def self.getFolderpathOrNull()
        folderpath = FIFOQueue::getFirstOrNull(CATALYST_COMMON_KEY_VALUE_STORE_REPOSITORY, "folderpaths-f363-4a11-9251-b7301406e261")
        if folderpath.nil? then
            JSON.parse(`ninja api:pending`).shuffle.each{|folderpath|
                FIFOQueue::push(CATALYST_COMMON_KEY_VALUE_STORE_REPOSITORY, "folderpaths-f363-4a11-9251-b7301406e261", folderpath)
            }
        end
        FIFOQueue::getFirstOrNull(CATALYST_COMMON_KEY_VALUE_STORE_REPOSITORY, "folderpaths-f363-4a11-9251-b7301406e261")
    end

    def self.getCatalystObjects()
        FIFOQueue::takeWhile(CATALYST_COMMON_KEY_VALUE_STORE_REPOSITORY, "timestamps-5bd4-431b-9eef-24ca1d005a3c", lambda{|unixtime| (Time.new.to_i - unixtime)>86400 })
        folderpath = Ninja::getFolderpathOrNull()
        if folderpath.nil? then
            return []
        end
        metric = 0.2 + 0.4*Math.exp(-FIFOQueue::size(CATALYST_COMMON_KEY_VALUE_STORE_REPOSITORY, "timestamps-5bd4-431b-9eef-24ca1d005a3c").to_f/16) + Saturn::traceToMetricShift("deb58288-31e9-4d20-848d-8ac33d3701ee")
        [
            {
                "uuid" => "96287511",
                "metric" => metric,
