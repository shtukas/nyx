...
class NxTodos
    ...
    # NxTodos::items()
    def self.items()
        Phage::objectsForMikuType("NxTodo")
    end
    ...
    def self.listingPriorityOrNull(item) # Float between 0 and 1
        ...
        if item["nx11e"]["type"] == "standard" and item["cx22"] and item["cx23"] then
            ...
            if cx22.nil? then
                ...
                Phage::setAttribute2(item["uuid"], "cx22", nil)
                return nil
            end
            ...
            Cx22::items().each{|cx22|
                NxTodos::items()
                    ...
                    .each{|item|
                        ...
                        Phage::setAttribute2(item["uuid"], "listeable", true)
                    }
            }
    ...
    def self.edit(item)
        if item["nx113"].nil? then
            ...
            if status then
                ...
                return Phage::getObjectOrNull(item["uuid"])
            else
                return item
            end
        end
        Nx113Edit::edit(item)
        Phage::getObjectOrNull(item["uuid"])
    end
    ...
    def self.landing(item)
        loop {
            ...
            if Interpreting::match("engine", input) then
                ...
                next if engine.nil?
                Phage::setAttribute2(item["uuid"], "nx11e", engine)
                next
            end
