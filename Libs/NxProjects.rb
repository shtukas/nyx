...
class NxProjects
    ...
    def self.toStringWithDetails(item, shouldFormat)
        ...
        dataStr = " (today: #{"%4.2f" % data["todayDoneHours"]} of #{"%4.2f" % data["todayDueHours"]} h, #{"%5.2f" % data["sinceWeekStartHoursDone"]} hss, need: #{"%5.2f" %  data["sinceWeekStartHoursIdeal"]}, #{data["isUpToDate"] ? "✨" :  "🔥"})"
