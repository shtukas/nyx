...
class Topping
    ...
    def self.getText(universe)
        ...
        filepath = "#{Config::pathToLocalDidact()}/Catalyst/Multiverse/tops/#{universe}.txt"
    ...
    def self.putTextToDataBank(text, universe)
        ...
        filepath = "#{Config::pathToLocalDidact()}/Catalyst/top-text-versions/#{Time.new.strftime("%Y")}/#{Time.new.strftime("%Y-%m")}/#{unixtime}-#{universe}.txt"
