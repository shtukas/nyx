...
class Transmutation
    ...
    def self.transmutation1(item, source, target, isSimulation = false)
        ...
        if source == "NxCollection" and target == "NxPerson" then
            ...
            Fx18Attributes::setAttribute2(item["uuid"], "name", item["description"])
            Fx18Attributes::setAttribute2(item["uuid"], "mikuType", "NxPerson")
            return
        end

        if source == "NxCollection" and target == "NxTimeline" then
            return true if isSimulation
            Fx18Attributes::setAttribute2(item["uuid"], "mikuType", "NxTimeline")
            return
        end

        if source == "NxDataNode" and target == "NxCollection" then
            return true if isSimulation
            Fx18Attributes::setAttribute2(item["uuid"], "mikuType", "NxCollection")
            return
        end

        if source == "NxDataNode" and target == "NxEvent" then
            return true if isSimulation
            Fx18Attributes::setAttribute2(item["uuid"], "datetime", CommonUtils::interactiveDateTimeBuilder())
            Fx18Attributes::setAttribute2(item["uuid"], "mikuType", "NxEvent")
            return
        end
        ...
        if source == "NxFrame" and target == "TxDated" then
            return true if isSimulation
            Fx18Attributes::setAttribute2(item["uuid"], "mikuType", "TxDated")
            Fx18Attributes::setAttribute2(item["uuid"], "datetime", CommonUtils::interactivelySelectDateTimeIso8601OrNullUsingDateCode())
            return
        end
        ...
        if source == "TxDated" and target == "NxFrame" then
            return true if isSimulation
            Fx18Attributes::setAttribute2(item["uuid"], "mikuType", "NxFrame")
            return
        end
