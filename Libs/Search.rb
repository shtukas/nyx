...
class Search
    ...
    def self.match(item, fragment)
        Nodes::description(item).downcase.include?(fragment.downcase)
    end
    ...
    def self.searchAndDive()
        loop {
            ...
            loop {
                ...
                selected = selected.select{|item| ItemsDatabase::itemOrNull(item["uuid"]) } # In case something has changed, we want the ones that have survived

                item = LucilleCore::selectEntityFromListOfEntitiesOrNull("item", selected, lambda{|i| i["description"] })
                break if item.nil?
                Nx27::programNode(item, false)
            }
        }
    end
end