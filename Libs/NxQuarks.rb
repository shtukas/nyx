...
# require "/Users/pascal/Galaxy/LucilleOS/Libraries/Ruby-Libraries/NxQuarks.rb"

=begin

Specs:

NxQuarks (NxA001, NxA002, etc) are immutable data carriers. They are self contained KV stores used by Catalyst and Nyx.
...
class NxQsUtils
    ...
    # NxQsUtils::findAxiomFilepathByIdOrNull(repositoryRoot, id)
    ...
    # NxQsUtils::editTextSynchronously(text)
    ...
    # NxQsUtils::destroy(repositoryRoot, id)
    def self.destroy(repositoryRoot, id)
        filepath = NxQsUtils::findAxiomFilepathByIdOrNull(repositoryRoot, id)
...
class NxA001
    ...
    def self.accessWithOptionToEdit(repositoryRoot, id)
        ...
        db.close
        text2 = NxQsUtils::editTextSynchronously(text)
        if text2 != text then
            db = Daybreak::DB.new filepath
...
class NxQuarks
    ...
    # NxQuarks::interactivelyCreateNewAxiom_EchoIdOrNull(fileParentFolder: String, id: String) : AxiomId or null
    def self.interactivelyCreateNewAxiom_EchoIdOrNull(fileParentFolder, id)
        ...
        if type == "text" then
            text = NxQsUtils::editTextSynchronously("")
    ...
    # NxQuarks::contentTypeOrNull(repositoryRoot: String, id: String)
    ...
    # NxQuarks::accessWithOptionToEdit(repositoryRoot: String, id: String)
    def self.accessWithOptionToEdit(repositoryRoot, id)
        return if id.nil?
        filepath = NxQsUtils::findAxiomFilepathByIdOrNull(repositoryRoot, id)
        return if filepath.nil?
    ...
    # NxQuarks::destroy(repositoryRoot: String, id: String | null): Boolean
    def self.destroy(repositoryRoot, id)
        return if id.nil?
        NxQsUtils::destroy(repositoryRoot, id)
    end

    # NxQuarks::fsck(repositoryRoot: String, id: String)
