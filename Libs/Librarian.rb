...
class Librarian
    ...
    def self.getObjectsByMikuTypeAndUniverse(mikuType, universe)
        ...
        db.results_as_hash = true
        answer = []
        db.execute("select * from _objects_ where _mikuType_=? and _universe_=?", [mikuType, universe]) do |row|
            answer << JSON.parse(row['_object_'])
        end
        db.close
        answer
    ...
    def self.commit(object)
        ...
        db.execute "insert into _objects_ (_objectuuid_, _mikuType_, _object_, _universe_) values (?, ?, ?, ?)", [object["uuid"], object["mikuType"], JSON.generate(object), object["universe"]]
        db.close
    end
