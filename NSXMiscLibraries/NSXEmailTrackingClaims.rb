...
class NSXEmailTrackingClaims
    ...
    def self.getClaimByEmailUIDOrNull(emailuid)
        claim = KeyValueStore::getOrNull(nil, "9bba7e64-2322-4e90-835f-5c4fa5929c87:#{emailuid}")
    ...
    def self.getClaimByGenericContentsItemUUIDOrNull(uuid)
        claim = KeyValueStore::getOrNull(nil, "9bba7e64-06e2-4999-a005-bbed7e9c1af3:#{uuid}")
        return nil if claim.nil?
        JSON.parse(claim)
    end
    ...
    def self.getClaimByStreamItemUUIDOrNull(uuid)
        claim = KeyValueStore::getOrNull(nil, "9bba7e64-e0eb-4dee-9340-be439f7891ff:#{uuid}")
        return nil if claim.nil?
        JSON.parse(claim)
    end
