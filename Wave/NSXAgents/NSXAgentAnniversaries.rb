...
class NSXAgentAnniversaries
    ...
    def self.getEventLines()
        IO.read("#{CATALYST_FOLDERPATH}/Wave/anniversaries/anniversaries.txt")
            .lines
            .map{|line| line.strip }
    ...
    def self.trueIfAnniversaryHasBeenProcessed(event, anniversary)
        KeyValueStore::flagIsTrue("#{CATALYST_FOLDERPATH}/Wave/anniversaries/kvstore-data", "b05b9dae-93d5-40f0-b68c-8cec95804b89:#{event["description"]}:#{JSON.generate(anniversary)}")
    end
    ...
    def self.markAnniversaryAsProcessed(event, anniversary)
        KeyValueStore::setFlagTrue("#{CATALYST_FOLDERPATH}/Wave/anniversaries/kvstore-data", "b05b9dae-93d5-40f0-b68c-8cec95804b89:#{event["description"]}:#{JSON.generate(anniversary)}")
    end
    ...
    # NSXAgentAnniversaries::processObjectAndCommand(objectuuid, command)
